...
class Model_Table extends CI_Model
{
    ...
    public function getBankTransactionSummary($accountId = null, $date = null)
    {
        $bankTransactionSummary = $this->db->query("
            select 
                ...
                (
                    select ifnull(sum(cp.CPayment_amount_bank), 0) from tbl_customer_payment cp
                ...
                (
                    select ifnull(sum(sp.SPayment_amount_bank), 0) from tbl_supplier_payment sp
    ...
    public function supplierDue($clauses = "", $date = null)
    {
        ...
        $supplierDues = $this->db->query("
            ...
            (select ifnull(sum(sp.SPayment_amount_cash + sp.SPayment_amount_bank), 0.00) from tbl_supplier_payment sp 
            ...
            (select ifnull(sum(sp2.SPayment_amount_cash + sp2.SPayment_amount_bank), 0.00) from tbl_supplier_payment sp2 
    ...
    public function customerDue($clauses = "", $date = null)
    {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(cp.CPayment_amount_cash + cp.CPayment_amount_bank), 0.00) 
