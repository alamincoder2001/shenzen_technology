...
class Account extends CI_Controller
{
    ...
    public function getBankAccounts()
    {
        $accounts = $this->db->query("SELECT *,
            concat(account_name, ' - ',account_number, ' (', bank_name, ')' ) as display_text,
        ...
        $transactions = $this->db->query("
            select * from(
                ...
                select
                    ...
                    cp.CPayment_amount_bank as deposit,
                    0.00 as withdraw,
                ...
                select
                    ...
                    0.00 as deposit,
                    cp.CPayment_amount_bank as withdraw,
                    cp.CPayment_notes as note,
                    ac.account_name,
                    ac.account_number,
                    ac.bank_name,
                ...
                select 
                    ...
                    'withdraw' as transaction_type,
                    0.00 as deposit,
                    sp.SPayment_amount_bank as withdraw,
                ...
                select 
                    ...
                    sp.SPayment_date as transaction_date,
                    'deposit' as transaction_type,
                    sp.SPayment_amount_bank as deposit,
                    0.00 as withdraw,
                    sp.SPayment_notes as note,
                    ac.account_name,
                    ac.account_number,
    ...
    public function getCashLedger()
    {
        ...
        $ledger = $this->db->query("
            ...
            select 
                ...
                cp.CPayment_amount_cash as in_amount,
                0.00 as out_amount
            from tbl_customer_payment cp
            join tbl_customer c on c.Customer_SlNo = cp.CPayment_customerID
            ...
            select 
                ...
                sp.SPayment_amount_cash as in_amount,
                0.00 as out_amount
            from tbl_supplier_payment sp
            join tbl_supplier s on s.Supplier_SlNo = sp.SPayment_customerID
            ...
            select 
                ...
                0.00 as in_amount,
                sp.SPayment_amount_cash as out_amount
            from tbl_supplier_payment sp 
            join tbl_supplier s on s.Supplier_SlNo = sp.SPayment_customerID
            ...
            select 
                ...
                0.00 as in_amount,
                cp.CPayment_amount_cash as out_amount
            from tbl_customer_payment cp
            join tbl_customer c on c.Customer_SlNo = cp.CPayment_customerID
