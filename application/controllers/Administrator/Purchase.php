    ...
    public function getPurchaseReturnDetails()
    {
        ...
        $returnDetails = $this->db->query("
            select 
                ...
                s.Supplier_Name
            from tbl_purchasereturndetails prd
            left join tbl_product p on p.Product_SlNo = prd.PurchaseReturnDetailsProduct_SlNo
            left join tbl_purchasereturn pr on pr.PurchaseReturn_SlNo = prd.PurchaseReturn_SlNo
        ...
        try {
            ...
            foreach ($data->cartProducts as $cartProduct) {
                ...
                if ($cartProduct->productId == 'P01') {
                    ...
                    $product['Product_Name']          = $cartProduct->name;
                    $product['Product_Purchase_Rate'] = $cartProduct->purchaseRate;
                    $product['Product_SellingPrice']  = $cartProduct->salesRate;
                    $product['ProductCategory_ID']    = 1;
                    $product['Unit_ID']               = 1;
                    $product['is_service']            = 'false';
                    $product['status']                = 'a';
                    $product['AddBy']                 = $this->session->userdata("FullName");
                    $product['AddTime']               = date('Y-m-d H:i:s');
                    $product['Product_branchid']      = $this->session->userdata('BRANCHid');

                    $this->db->insert('tbl_product', $product);
                    $Product_Id = $this->db->insert_id();
                }

                $prodId = $cartProduct->productId == 'P01' ? $Product_Id : $cartProduct->productId;

                $purchaseDetails = array(
                    'PurchaseMaster_IDNo'           => $purchaseId,
                    'Product_IDNo'                  => $prodId,
                    'PurchaseDetails_TotalQuantity' => $cartProduct->quantity,
                    'PurchaseDetails_Rate'          => $cartProduct->purchaseRate,
                    'PurchaseDetails_TotalAmount'   => $cartProduct->total,
                    'Status'                        => 'a',
                    'AddBy'                         => $this->session->userdata("FullName"),
                    'AddTime'                       => date('Y-m-d H:i:s'),
                ...
                $previousStock = $this->mt->productStock($prodId);

                $inventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ?", [$prodId, $this->session->userdata('BRANCHid')])->num_rows();
                if ($inventoryCount == 0) {
                    $inventory = array(
                        'product_id'        => $prodId,
                        'purchase_quantity' => $cartProduct->quantity,
                    ...
                    $this->db->query("
                        ...
                        where product_id = ?
                        and branch_id = ?
                    ", [$cartProduct->quantity, $prodId, $this->session->userdata('BRANCHid')]);
                }
                ...
                ", [
                    $previousStock,
                    $cartProduct->total,
                    ($previousStock + $cartProduct->quantity),
                    $cartProduct->salesRate,
                    $prodId
                ]);
            }
    ...
    public function updatePurchase()
    {
        ...
        try {
            ...
            foreach ($data->cartProducts as $product) {
                if ($product->productId == 'P01') {
                    $products                          = [];
                    $products['Product_Code']          = $this->mt->generateProductCode();
                    $products['Product_Name']          = $product->name;
                    $products['Product_Purchase_Rate'] = $product->purchaseRate;
                    $products['Product_SellingPrice']  = $product->salesRate;
                    $products['ProductCategory_ID']    = 1;
                    $products['Unit_ID']               = 1;
                    $products['is_service']            = 'false';
                    $products['status']                = 'a';
                    $products['AddBy']                 = $this->session->userdata("FullName");
                    $products['AddTime']               = date('Y-m-d H:i:s');
                    $products['Product_branchid']      = $this->session->userdata('BRANCHid');

                    $this->db->insert('tbl_product', $product);
                    $Product_Id = $this->db->insert_id();
                }

                $prodId = $product->productId == 'P01' ? $Product_Id : $product->productId;
