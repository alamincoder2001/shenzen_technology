...
<div id="customerPayment">
	<div class="row" style="border-bottom: 1px solid #ccc;padding: 15px 0;margin-bottom: 15px;">
		<div class="col-md-12">
				...
				<div class="row">
					...
					<div class="col-md-5">
						...
						<div class="form-group">
							...
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="number" class="form-control" v-model="payment.CPayment_amount_cash" required>
							</div>
						</div>
						<div class="form-group">
							<label class="col-md-4 control-label">Bank Amount</label>
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="number" class="form-control" v-model="payment.CPayment_amount_bank" required>
							</div>
						</div>
						<div class="form-group" style="display:none;" v-bind:style="{display: payment.CPayment_amount_bank > 0 ? '' : 'none'}">
							<label class="col-md-4 control-label">Bank Account</label>
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<v-select v-bind:options="filteredAccounts" v-model="selectedAccount" label="display_text" placeholder="Select account"></v-select>
	...
	new Vue({
		...
		methods: {
			...
			saveCustomerPayment() {
				...
				if (this.payment.CPayment_amount_bank > 0 && this.selectedAccount.account_id == '') {
					alert('Select an account');
					return;
				} else {
					this.payment.account_id = this.payment.CPayment_amount_bank > 0 ? this.selectedAccount.account_id : null;
				}
			...
			editPayment(payment) {
				...
				if (payment.CPayment_amount_bank > 0) {
			...
			resetForm() {
				...
				this.payment.CPayment_amount_cash = 0;
				this.payment.CPayment_amount_bank = 0;
